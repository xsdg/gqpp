...
{
    fd->sidecar->disable_grouping(fd, disable);
}

void file_data_disable_grouping_list(GList *fd_list, gboolean disable)
{
    FileData *fd = first_fd_from_list(fd_list);
    if (fd) fd->sidecar->disable_grouping_list( fd_list, disable);
}
...
{
    return fd->sidecar->sc_list_to_string(fd);
}


gchar *file_data_get_sidecar_path(FileData *fd, gboolean existing_only)
{
    return fd->sidecar->get_sidecar_path(fd, existing_only);
}
...
{
    FileData *fd = first_fd_from_list(list);
    if (!fd) return NULL;
    return fd->sidecar->process_groups_in_selection(list, ungroup, ungrouped);
}
