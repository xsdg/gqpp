...
hard_coded_window_keys collection_window_keys[] = {
	...
	{NO_GDK_MODIFIER, 'A', N_("Append from collection")},
	{NO_GDK_MODIFIER, 'S', N_("Save collection")},
	{GDK_CONTROL_MASK, 'S', N_("Save collection as")},
	{GDK_CONTROL_MASK, 'T', N_("Show filename text")},
	{NO_GDK_MODIFIER, 'N', N_("Sort by name")},
	{NO_GDK_MODIFIER, 'I', N_("Sort by number")},
	{NO_GDK_MODIFIER, 'D', N_("Sort by date")},
	{NO_GDK_MODIFIER, 'B', N_("Sort by size")},
	{NO_GDK_MODIFIER, 'P', N_("Sort by path")},
...
static CollectInfo *collection_table_find_data_by_coord(CollectTable *ct, gint x, gint y, GtkTreeIter *iter)
{
	...
	if (iter) *iter = row;
	return (CollectInfo *)g_list_nth_data(list, n);
}
...
static void tip_update(CollectTable *ct, CollectInfo *info)
{
	...
	if (ct->tip_window)
		...
		if (info != ct->tip_info)
			...
			label = (GtkWidget *)g_object_get_data(G_OBJECT(ct->tip_window), "tip_label");
...
static void collection_table_popup_edit_cb(GtkWidget *widget, gpointer data)
{
	...
	ct = (CollectTable *)submenu_item_get_data(widget);
						 ...
						 gboolean use_coord, gint x, gint y)
{
	...
	if (gtk_tree_view_get_path_at_pos(GTK_TREE_VIEW(ct->listview), x, y,
		...
		n = GPOINTER_TO_INT(g_object_get_data(G_OBJECT(column), "column_number"));
		info = (CollectInfo *)g_list_nth_data(list, n);
...
static CollectInfo *collection_table_insert_point(CollectTable *ct, gint x, gint y)
{
	...
	if (info && after)
		...
		if (work && work->next)
			{
			info = (CollectInfo *)work->next->data;
			}
		else
			{
			info = NULL;
					  ...
					  GtkTreeModel *tree_model, GtkTreeIter *iter, gpointer data)
{
	...
	info = (CollectInfo *)g_list_nth_data(list, cd->number);
