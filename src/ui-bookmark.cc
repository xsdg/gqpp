...
static void bookmark_select_cb(GtkWidget *button, gpointer data)
{
	...
	b = (BookButtonData *)g_object_get_data(G_OBJECT(button), "bookbuttondata");
	if (!b) return;
...
void bookmark_list_set_key(GtkWidget *list, const gchar *key)
{
	...
	bm = (BookMarkData *)g_object_get_data(G_OBJECT(list), BOOKMARK_DATA_KEY);
	if (!bm) return;
...
void history_combo_append_history(GtkWidget *widget, const gchar *text)
{
	...
	hc = (HistoryComboData *)g_object_get_data(G_OBJECT(widget), "history_combo_data");
	if (!hc)
		{
