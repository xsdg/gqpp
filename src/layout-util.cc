...
static void layout_menu_split_cb(GtkRadioAction *action, GtkRadioAction *UNUSED(current), gpointer data)
{
	...
	layout_exit_fullscreen(lw);
	mode = (ImageSplitMode)gtk_radio_action_get_current_value(action);
...
static void layout_menu_overlay_cb(GtkToggleAction *action, gpointer data)
{
	...
	if (gtk_toggle_action_get_active(action))
		...
		if ((flags | OSD_SHOW_INFO | OSD_SHOW_STATUS) != flags)
			image_osd_set(lw->image, (OsdShowFlags)(flags | OSD_SHOW_INFO | OSD_SHOW_STATUS));
		}
	else
		{
...
static void layout_menu_histogram_cb(GtkToggleAction *action, gpointer data)
{
	...
	if (gtk_toggle_action_get_active(action))
		{
		image_osd_set(lw->image, (OsdShowFlags)(OSD_SHOW_INFO | OSD_SHOW_STATUS | OSD_SHOW_HISTOGRAM));
		...
		if (flags & OSD_SHOW_HISTOGRAM)
			image_osd_set(lw->image, (OsdShowFlags)(flags & ~OSD_SHOW_HISTOGRAM));
		}
}
		...
		while (keymap_template[keymap_index])
			...
			if (g_strrstr(keymap_template[keymap_index], ">key:"))
				...
				for (index=0; index < array->len-2; index=index+2)
					{
					if (!(g_ascii_strcasecmp((gchar *)g_ptr_array_index(array,index+1), post_key[0])))
						{
						key_name = (gchar *)g_ptr_array_index(array,index+0);
						break;
						}
					}
...
void layout_actions_setup(LayoutWindow *lw)
{
	...
	for (i = 0; i < TOOLBAR_COUNT; i++)
		{
		layout_toolbar_clear(lw, (ToolbarType)i);
		layout_toolbar_add_default(lw, (ToolbarType)i);
		}
...
static gboolean layout_editors_reload_idle_cb(gpointer UNUSED(data))
{
	...
	editor_read_desktop_file((gchar *)layout_editors_desktop_files->data);
