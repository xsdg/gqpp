...
static OverlayStateData *image_get_osd_data(ImageWindow *imd)
{
	...
	osd = (OverlayStateData *)g_object_get_data(G_OBJECT(imd->pr), "IMAGE_OVERLAY_DATA");
	return osd;
}
...
{
	OverlayStateData *osd = (OverlayStateData*)image_get_osd_data(imd);
...
static GdkPixbuf *image_osd_info_render(OverlayStateData *osd)
{
	...
	if (name)
		...
		osd_template_insert(vars, "date", g_strdup(imd->image_fd ? text_from_time(imd->image_fd->date) : ""), OSDT_NONE);
